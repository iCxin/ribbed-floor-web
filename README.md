...
| 计算报告（含结构简图） | 计算书 | 软著证书 |
| --- | --- | --- |
| ![计算报告](docs/screenshot-report.png) | ![计算书](docs/screenshot-calcbook.png) | ![软著证书](docs/screenshot-copyright.png) |

## 功能

- **设计计算** —— 强度等级 C20~C80、单向/双向板、几何与荷载参数 → 完整设计报告
  （弯矩包络、挠度/裂缝验算、四部位配筋方案、用钢量、上下行协议帧十六进制）
- **结构简图** —— SVG 实时绘制：网格剖分、四边简支支座符号、尺寸标注（Lx/Ly）、
  受力方向示意与剖面（h）
- **计算书导出** —— 完整计算过程（工程概况 → 计算模型 → 荷载组合 → 内力分析 →
  挠度/裂缝验算逐步算式 → 分部位配筋计算 → 构造检查 → 结论），每一步均给出
  公式与代入数值，一键"打印 / 导出 PDF"生成正式计算书
...
- **软件著作权** —— 已取得国家版权局计算机软件著作权登记
  （登记号 2026SR0518833，著作权人：华南农业大学），页眉/页脚点击可查看证书
